                ...
                <div class="col-lg-6">
                    <table class="table table-borderless">
                        <tr>
                            ...
                            <td>{{ $order->status ? 'Active' : 'Cancel' }}</td>
                        </tr>
                        <tr>
    ...
    <div class="row">
        <div class="col-lg-9">
            ...
            @if (count($order->transactions))                
                <div class="card mt-4">
                    <div class="card-header">
                    <b class="fs-15">{{ translate('Transaction Details') }}</b>
                    </div>
                    <div class="card-body pb-0">
                        <table class="table table-borderless table-responsive">
                            <thead>
        ...
        <div class="col-lg-3">
                ...
                <div class="card-body pb-0">
                    <table class="table table-borderless">
                        <tbody>
                            ...
                            @if ($order->payment_status != 'paid' && !($order->manual_payment && $order->manual_payment_data == null))
                                @php
                                    $paid = $order->transactions ? $order->transactions->sum('amount') : 0;
                                @endphp
                                <tr>
                                    <td class="w-50 fw-600">{{ translate('Paid')}}</td>
                                    <td class="text-right">
                                        <strong><span>{{ single_price($paid) }}</span></strong>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="w-50 fw-600">{{ translate('Due')}}</td>
                                    <td class="text-right">
                                        <strong><span>{{ single_price($order->grand_total - $paid) }}</span></strong>
                                    </td>
                                </tr>
                            @endif
                        </tbody>
            ...
            @if ($order->payment_status != 'paid' && !($order->manual_payment && $order->manual_payment_data == null))
                <button onclick="show_partial_payment_modal({{ $order->id }})" class="btn btn-block btn-primary">{{ translate('Make Payment')}}</button>
            @endif
...
<script type="text/javascript">
    function show_partial_payment_modal(order_id){
        $.post('{{ route('checkout.make_partial_payment') }}', {_token:'{{ csrf_token() }}', order_id : order_id}, function(data){
            $('#partial_payment_modal_body').html(data);
