    ...
    <div class="aiz-user-sidenav rounded overflow-hidden  c-scrollbar-light">
        ...
        <div class="px-4 text-center mb-4">
            @if(Auth::user()->user_type == 'customer')
                <img src="{{ Auth::user()->avatar_original ? uploaded_asset(Auth::user()->avatar_original) : static_asset('assets/img/avater.png') }}" alt="" class="rounded-circle" width="150" height="150">
            @else
                <img src="{{ Auth::user()->avatar_original ? uploaded_asset(Auth::user()->avatar_original) : static_asset('assets/img/avater.png') }}" alt="" class="rounded-circle" width="150" height="150">
            @endif
            <hr>
            @if(Auth::user()->user_type == 'customer')
                <h4 class="h5 fw-600">Hello, {{ Auth::user()->name }}</h4>
            @else
                <h4 class="h5 fw-600">{{ Auth::user()->name }}
                    <span class="ml-2">
                        @if(Auth::user()->seller->verification_status == 1)
                        <img class="w-25px h-25px" src="{{ static_asset('assets/img/checkmark.png') }}" alt="">
                        @else
                            <img class="w-25px h-25px" src="{{ static_asset('assets/img/shield.png') }}" alt="">
                        @endif
                    </span>
                </h4>
            @endif
            ...
            <ul class="aiz-side-nav-list" data-toggle="aiz-side-menu">
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('purchase_history.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['purchase_history.index'])}}">
                        <span class="aiz-side-nav-text">{{ translate('Order List') }}</span>
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('digital_purchase_history.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['digital_purchase_history.index'])}}">
                        <span class="aiz-side-nav-text">{{ translate('Downloadable Orders') }}</span>
                    </a>
                </li>
                ...
                @if(Auth::user()->user_type == 'seller')
                    ...
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('orders.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['orders.index'])}}">
                            <span class="aiz-side-nav-text">{{ translate('Product Orders') }}</span>
