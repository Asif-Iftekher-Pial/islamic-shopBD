                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                ...
                                @else
                                    <span class="badge badge-inline badge-info">{{ translate('Cancel') }}</span>
                                @endif
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-main text-bold">{{translate('Total Paid')}}</td>
                            <td class="text-right">
                              {{ single_price($order->paid) }}
                            </td>
                        </tr>
                        <tr>
                            <td class="text-main text-bold">{{translate('Total Due')}}</td>
                            <td class="text-right">
    		...
    		<div class="">
    				<table class="table table-bordered aiz-table">
                        ...
                        <th data-breakpoints="lg" class="text-uppercase">{{translate('Delivery Type')}}</th>
                                ...
                                <th data-breakpoints="lg" class="min-col text-right text-uppercase">{{translate('Paid')}}</th>
                                <th data-breakpoints="lg" class="min-col text-right text-uppercase">{{translate('Due')}}</th>
            				</tr>
        				</thead>
        				<tbody>
                    @php
                    ...
                    @foreach ($order->orderDetails->where('seller_id', '!=', $admin_user_id) as $key => $orderDetail)
                        <tr>
                            ...
                            <td class="text-center">{{ single_price($orderDetail->paid) }}</td>
                            <td class="text-center">{{ single_price($orderDetail->price - $orderDetail->paid) }}</td>
                        </tr>
                    @endforeach
        				</tbody>
    				</table>
    		...
    		<div class="clearfix float-right">
    			<table class="table">
          			<tbody>
                        ...
                        <tr>
            				<td><strong class="text-muted"> {{translate('Shipping')}} ({{ json_decode($order->shipping_address)->city }}) :</strong></td>
                        ...
                        <tr>
            				<td><strong class="text-muted">{{translate('Paid')}} :</strong></td>
            				<td class="text-muted h5">
            					{{ single_price($order->paid) }}
            				</td>
            			</tr>
                        <tr>
            				<td><strong class="text-muted">{{translate('Due')}} :</strong></td>
            				<td class="text-muted h5">
            					{{ single_price($order->grand_total - $order->paid) }}
            				</td>
            			</tr>
