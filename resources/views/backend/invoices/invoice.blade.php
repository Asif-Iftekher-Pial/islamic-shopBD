...
<body>
	<div>
		...
		<div style="background: #ffffff;padding: 1rem;">
			<table>
				<tr>
					<td style="font-size: 1.5rem;" class="strong">{{  translate('INVOICE') }}</td>
			...
			<table>
				<tr>
					<td></td>
					<td style="font-size: 1rem;" class="text-right strong">{{ get_setting('site_name') }}</td>
				</tr>
				<tr>
					<td></td>
					<td class="gry-color small text-right" style="width: 400px !important;">{{ get_setting('contact_address') }}</td>
				</tr>
				<tr>
					<td class="small"><span class="gry-color small">{{  translate('Order ID') }}:</span> <span class="strong">{{ $order->code }}</span></td>
					<td class="gry-color small text-right">{{  translate('Email') }}: {{ get_setting('contact_email') }}</td>
				</tr>
				<tr>
					<td class="small"><span class="gry-color small">{{  translate('Order Date') }}:</span> <span class=" strong">{{ date('d-m-Y', $order->date) }}</span></td>
					<td class="gry-color small text-right">{{  translate('Phone') }}: {{ get_setting('contact_phone') }}</td>
				</tr>
			</table>
		...
		<div style="padding: 1rem;padding-bottom: 0">
            <table>
				...
				<tr><td class="gry-color small">{{ $shipping_address->address }}, {{ isset($shipping_address->ccity) ? $shipping_address->ccity : '' }}, {{ $shipping_address->country }}</td></tr>
	    ...
	    <div style="padding: 1rem;">
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr class="gry-color" style="background: #ffffff;">
			            ...
			            <td>
					        <table class="text-right sm-padding small strong">
						        <tbody>
							        ...
							        <tr>
							            <th class="gry-color text-left">{{ translate('Shipping Cost') }} ( {{ $shipping_address->city }} )</th>
							        ...
							        <tr>
							            <th class="text-left strong">{{ translate('Total Paid') }}</th>
							            <td class="currency">{{ single_price($order->transactions->sum('amount')) }}</td>
							        </tr>
							        <tr>
							            <th class="text-left strong">{{ translate('Total Due') }}</th>
							            <td class="currency">{{ single_price($order->grand_total - $order->transactions->sum('amount')) }}</td>
							        </tr>
									@endif
						        </tbody>
						    </table>
