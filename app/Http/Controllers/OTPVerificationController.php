...
class OTPVerificationController extends Controller
{
    ...
    public function send_order_code($order){
        if(json_decode($order->shipping_address)->phone != null){
            sendSMS(json_decode($order->shipping_address)->phone, env('APP_NAME'), 'Your '.env('APP_NAME').' order has been placed and Invoice No. is : '.$order->code);
        }
    }
    ...
    public function send_delivery_status($order){
        if(json_decode($order->shipping_address)->phone != null){
            sendSMS(json_decode($order->shipping_address)->phone, env('APP_NAME'), 'Your '.env('APP_NAME').' delivery status has been updated to '.$order->orderDetails->first()->delivery_status.' for Invoice No. : '.$order->code);
        }
    }
    ...
    public function send_payment_status($order){
        if(json_decode($order->shipping_address)->phone != null){
            sendSMS(json_decode($order->shipping_address)->phone, env('APP_NAME'), 'Your '.env('APP_NAME').' payment status has been updated to '.$order->payment_status.' for Invoice No. : '.$order->code);
        }
    }
}
