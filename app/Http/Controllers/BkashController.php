...
class BkashController extends Controller
{
    ...
    public function pay($pay_amount = null){
        $amount = 0;
        if(Session::has('payment_type')){
            if(Session::get('payment_type') == 'cart_payment'){
                $order = Order::findOrFail(Session::get('order_id'));
                // $amount = round(BusinessSetting::where('type', 'partial_system')->first()->value == 1 ? $order->paid : $order->grand_total);
                $amount = $pay_amount;
            }
    ...
    public function success(Request $request){
        ...
        if ($payment_type == 'cart_payment') {
            ...
            return $checkoutController->checkout_done(Session::get('order_id'), $request->payment_details, $amount, 'bkash');
        }

        if ($payment_type == 'partial') {
            $partialController = new PartialPaymentController;
            return $partialController->partial_payment_done(Session::get('order_id'), $request->payment_details, $amount, 'bkash');
        }
