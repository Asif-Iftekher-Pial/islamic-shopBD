...
class NagadController{
    ...
    public function getSession($pay_amount = null)
    {
        if(Session::has('payment_type')){
            if(Session::get('payment_type') == 'cart_payment'){
                $order = Order::findOrFail(Session::get('order_id'));
                // $this->amount(BusinessSetting::where('type', 'partial_system')->first()->value == 1 ? $order->paid : $order->grand_total);
    ...
    public function verify(Request $request){
        ...
        if(json_decode($json)->status == 'Success'){
            ...
            if ($payment_type == 'cart_payment') {
                ...
                return $checkoutController->checkout_done(Session::get('order_id'), $json, $this->amount, 'Nagad');
            }
            if ($payment_type == 'partial') {
                $partialController = new PartialPaymentController;
                return $partialController->partial_payment_done(Session::get('order_id'), $json, $this->amount, 'Nagad');
            }
